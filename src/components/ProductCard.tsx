...
export default function ProductCard({ product }: { product: any }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow border border-gray-100 overflow-hidden group relative">
      ...
      <div className="p-5">
        <p className="text-sm text-brand-teal font-semibold mb-1">{product.modelNumber}</p>
        ...
        <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-50">
          <span className="text-xl font-bold text-brand-red">{product.price} ج.م</span>
          <button 
            onClick={() => setShowQuickAdd(true)}
            className="flex items-center gap-2 bg-brand-teal text-white px-4 py-2 rounded-lg font-semibold hover:bg-opacity-90 transition-colors"
          >
      ...
      {showQuickAdd && (
        <div className="absolute inset-0 bg-white/95 z-10 p-5 flex flex-col backdrop-blur-sm transition-all">
          <button 
            ...
            className="absolute top-3 left-3 text-gray-500 hover:text-brand-red"
          >
            <X className="w-6 h-6" />
          </button>
          ...
          <div className="flex-grow space-y-4 overflow-y-auto">
            <div>
              <p className="text-sm font-semibold mb-2 text-brand-teal">اللون:</p>
                ...
                {(product.colors || []).map((c: any, i: number) => (
                  <button
                    ...
                    className={`px-3 py-1 text-sm rounded border ${
                      selectedColor === c.name ? "bg-brand-teal text-white border-brand-teal" : "border-gray-300 text-gray-700"
                    }`}
                  >
                    {c.name}
                  </button>
            ...
            {product.sizes && product.sizes.length > 0 && (
              <div>
                <p className="text-sm font-semibold mb-2 text-brand-teal">المقاس:</p>
                <div className="flex flex-wrap gap-2">
                  {product.sizes.map((s: string, i: number) => (
                    <button
                      ...
                      className={`w-10 h-10 flex items-center justify-center text-sm rounded border ${
                        selectedSize === s ? "bg-brand-red text-white border-brand-red" : "border-gray-300 text-gray-700"
                      }`}
                    >
                      {s}
                    </button>
          ...
          <button 
            ...
            className="w-full bg-brand-red text-white py-3 rounded-lg font-bold mt-4 hover:bg-opacity-90"
          >
            أضف إلى السلة
          </button>
        </div>
